...
class Dummy(Enemy):
    ...
    def __init__(self, level=1000000, name="dummy", characteristics=
    dummy_characteristics, stats=None,
    description="Testing dummy, it has so much hp " +
    "you can hit in endlessly. Also skips every turn.",
    inventory=[], equipment=default_equipment, tags=[], abilities=[],
    modifiers=[], exp_value=0):
...
class Rat(Enemy):
    ...
    def __init__(self, level=1, name="rat",
        characteristics=rat_characteristics, stats=None,
        description="An angry grey rat.", inventory=[],
        equipment=default_equipment,
        tags=["animate", "living", "rodent", "animal", "small"],
        abilities=[], modifiers=[], exp_value=50):
        Enemy.__init__(self, name, level, characteristics, stats,
        description, inventory, equipment, tags, abilities,
        modifiers, exp_value)
    ...
    def act(self, combat_event):
        ...
        if self.target and not self.target.dead:
            while self.energy >= self.abilities[0].energy_required:
                attack_infos.append(self.abilities[0].__class__.use(
                    self, self.target, self.primary_weapon, combat_event))
                if not self.target or self.target.dead:
                    break
...
class BigRat(Enemy):
    ...
    def __init__(self, level=1, name="big rat",
        characteristics=big_rat_characteristics, stats=None,
        description="A big angry grey rat.", inventory=[],
        equipment=default_equipment, tags=["living", "animate", "rodent",
        "animal", "small"], abilities=[], modifiers=[], exp_value=80):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        teeth = get_item_by_name("rodent_teeth", 0)
        if self.add_to_inventory(teeth):
    ...
    def act(self, combat_event):
        ...
        if self.target and not self.target.dead:
            while self.energy >= self.abilities[0].energy_required:
                attack_infos.append(self.abilities[0].__class__.use(self,
                self.target, self.primary_weapon, combat_event))
                if not self.target or self.target.dead:
                    break
...
class Wolf(Enemy):
    ...
    def __init__(self, level=1, name="wolf",
        characteristics=wolf_characteristics, stats=None,
        description="An angry grey wolf.", inventory=[],
        equipment=default_equipment, tags=["living", "animate", "animal"],
        ...
        if self.target and not self.target.dead:
            while self.energy >= self.abilities[0].energy_required:
                if random.randint(0, 1) == 1:
                    ...
                    attack_infos.append(self.abilities[1].__class__.use(self,
                        self.target, self.secondary_weapon, combat_event))
                if not self.target or self.target.dead:
                    break
...
class WolfLeader(Enemy):
    ...
    def __init__(self, level=1, name="wolf pack leader",
        characteristics=wolf_leader_characteristics, stats=None,
        description="An angry grey wolf.", inventory=[],
        equipment=default_equipment,
        tags=["living", "animate", "animal", "quick"], abilities=[],
        modifiers=[], exp_value=300):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        teeth = get_item_by_name("animal_teeth", 0)
        claws = get_item_by_name("animal_claws", 0)
...
class Bear(Enemy):
    ...
    def __init__(self, level=1, name="bear",
        characteristics=bear_characteristics, stats=None,
        description="An angry big bear. Very dangerous!", inventory=[],
        equipment=default_equipment, tags=["living", "animate", "animal",
        "big"], abilities=[], modifiers=[], exp_value=300):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        teeth = get_item_by_name("animal_teeth", 0)
        claws = get_item_by_name("animal_claws", 0)
...
class UndeadSoldier(Enemy):
    ...
    def __init__(self, level=1, name="undead soldier",
        characteristics=undead_soldier_characteristics,
        stats=None, description="An undead soldier.", inventory=[],
        equipment=default_equipment, tags=["animate", "humanoid", "undead",
        "slow"], abilities=[], modifiers=[], exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name(random.choice(["club", "sword", "dagger",
            "mace", "claymore", "rapier"]), 0)]
        items.append(get_item_by_name("targe shield", 0)) if random.randint(0,
            10) > 7 else None
        items.append(get_item_by_name(random.choice(["chainmail",
            "plate armor", "iron helmet"]), 0)) if random.randint(0,
                10) > 8 else None
        for item in items:
...
class UndeadSiren(Enemy):
    ...
    def __init__(self, level=1, name="undead siren",
        characteristics=undead_siren_characteristics, stats=None,
        description="A horribly misfigured woman. It seems to be abducted " +
        "by something evil.", inventory=[], equipment=default_equipment,
        tags=["animate", "humanoid", "undead", "slow"], abilities=[],
        modifiers=[], exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
...
class UndeadLegionaire(Enemy):
    ...
    def __init__(self, level=1, name="undead legionaire",
        characteristics=undead_legionaire_characteristics, stats=None,
        description="An undead legionaire.", inventory=[],
        equipment=default_equipment, tags=["animate", "humanoid", "undead",
        "slow"], abilities=[], modifiers=[], exp_value=200):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name(random.choice(["steel spear",
            "steel halberd"]), 0)]
        items.append(get_item_by_name(random.choice(["targe shield",
            "tower shield"]), 0)) if random.randint(0, 10) > 7 else None
        items.append(get_item_by_name(random.choice(["chainmail",
            "plate armor", "iron helmet"]), 0)) if random.randint(0,
            10) > 8 else None
        for item in items:
...
class UndeadWarLeader(Enemy):
    ...
    def __init__(self, level=1, name="undead war leader",
        characteristics=undead_warleader_characteristics, stats=None,
        description="A two and half meters high undead soldier. He carries a " +
        "torn banner that has long lost it's colors. He seems sentinent and " +
        "sane. His will to to dismember you is logically calculated.",
        inventory=[], equipment=default_equipment, tags=["animate", "humanoid",
        "undead", "big"], abilities=[], modifiers=[], exp_value=500):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name(random.choice(["sword", "claymore", "rapier",
            "mace", "steel spear", "stone maul", "steel halberd"]), 0)]
        items.append(get_item_by_name(random.choice(["targe shield", "dagger"]),
            0)) if random.randint(0, 10) > 7 else None
        items.append(get_item_by_name(random.choice(["ritual cloak",
            "golden crown"]), 0)) if random.randint(0, 10) > 8 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
    ...
    def act(self, combat_event):
        ...
        if self.target and not self.target.dead:
            for ability in self.abilities:
                if ability.__class__ == FearScream and "fear" in [x.name
                    for x in self.target.modifiers]:
                    continue
                while self.energy >= ability.energy_required:
                    attack_infos.append(ability.__class__.use(self,
                        self.target, ability.granted_by, combat_event))
...
class UndeadNinjaPirate(Enemy):
    ...
    def __init__(self, level=1, name="undead ninja pirate",
        characteristics=undead_ninja_pirate_characteristics, stats=None,
        description="An undead ninja pirate.", inventory=[],
        equipment=default_equipment, tags=["animate", "humanoid", "undead",
        "slow"], abilities=[], modifiers=[], exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
...
class UndeadKnight(Enemy):
    ...
    def __init__(self, level=1, name="undead knight",
        characteristics=undead_knight_characteristics, stats=None,
        description="An undead knight.", inventory=[],
        equipment=default_equipment, tags=["animate", "humanoid", "undead",
        "slow"], abilities=[], modifiers=[], exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
...
class Lich(Enemy):
    ...
    def __init__(self, level=1, name="lich",
        characteristics=lich_characteristics, stats=None,
        description="A lich.", inventory=[], equipment=default_equipment,
        tags=["animate", "humanoid", "undead"], abilities=[], modifiers=[],
        exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        ...
        items.append(get_item_by_name("targe shield", 0)) if random.randint(0,
            10) > 7 else None
        items.append(get_item_by_name("armor", 0)) if random.randint(0,
            10) > 2 else None
        items.append(get_item_by_name("headwear", 0)) if random.randint(0,
            10) > 2 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class LichCrystaline(Enemy):
    ...
    def __init__(self, level=1, name="crystaline",
        characteristics=crystaline_characteristics, stats=None,
        description="A huge crystal, it glows with various colors.",
        inventory=[], equipment=default_equipment, tags=[], abilities=[],
        modifiers=[], exp_value=400):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
...
class LesserDemon(Enemy):
    ...
    def __init__(self, level=1, name="lesser demon",
        characteristics=lesser_demon_characteristics, stats=None,
        description="A lesser demon.", inventory=[],
        equipment=default_equipment, tags=["living", "animate", "demon"],
        abilities=[], modifiers=[], exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = []
...
class BetaDemon(Enemy):
    ...
    def __init__(self, level=1, name="beta demon",
        characteristics=beta_demon_characteristics, stats=None,
        description="An beta demon. It's huge and wields a club from a huge " +
        "bone.", inventory=[], equipment=default_equipment, tags=["animate",
        "living", "demon", "slow", "big"], abilities=[], modifiers=[],
        exp_value=300):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = []
...
class Thug(Enemy):
    ...
    def __init__(self, level=1, name="thug",
        characteristics=thug_characteristics, stats=None,
        description="A thug, strong and massive, but quite slow.",
        inventory=[], equipment=default_equipment, tags=["animate", "humanoid",
        "slow", "big", "human", "living"], abilities=[], modifiers=[],
        exp_value=200):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name(random.choice(["club", "sword", "mace",
            "claymore"]), 0)]
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class Peasant(Enemy):
    ...
    def __init__(self, level=1, name="peasant",
        characteristics=peasant_characteristics, stats=None,
        description="A peasant turned bandit.", inventory=[],
        equipment=default_equipment, tags=["human", "living", "animate",
        "humanoid"], abilities=[], modifiers=[], exp_value=100):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name(random.choice(["club", "dagger", "mace",
            "quaterstaff"]), 0)]
        items.append(get_item_by_name("targe shield", 0)) if random.randint(0,
            10) > 7 else None
        items.append(get_item_by_name(random.choice(["chainmail", "plate armor",
            "iron helmet"]), 0)) if random.randint(0, 10) > 8 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class NinjaPirate(Enemy):
    ...
    def __init__(self, level=1, name="young pirate-ninja",
        ...
        Enemy.__init__(self, name, level, characteristics, stats, description,
             inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name(random.choice(["club", "dagger",
         "mace", "quaterstaff"]), 0)]
        items.append(get_item_by_name("targe shield", 0)) if random.randint(0,
        10) > 7 else None
        items.append(get_item_by_name(random.choice(["chainmail", "plate armor",
    ...
    def act(self, combat_event):
        ...
        if self.target and not self.target.dead:
            for ability in self.abilities:
                while self.energy >= ability.energy_required:
                    attack_infos.append(ability.__class__.use(self, self.target,
                        ability.granted_by, combat_event))
                    if not self.target or self.target.dead:
                        break
                if not self.target or self.target.dead:
                        break
...
class Mercenary(Enemy):
    ...
    def __init__(self, level=1, name="mercenary",
        characteristics=mercenary_characteristics, stats=None,
        description="Sword for hire. Somewhat professional, quite " +
        "experienced. Usually hired as canon foder and not really relied on.",
        inventory=[], equipment=default_equipment, tags=["human", "living",
        "animate", "humanoid"], abilities=[], modifiers=[], exp_value=200):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name("primary weapon", 0)]
        items.append(get_item_by_name("secondary weapon", 0)
            ) if random.randint(0, 10) > 3 else None
        items.append(get_item_by_name(random.choice(["leather armor",
            "chainmail", "plate armor"]), 0)) if random.randint(0,
            10) > 3 else None
        items.append(get_item_by_name(random.choice(["headwear"]), 0)
            ) if random.randint(0, 10) > 3 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class MercenarySpearman(Enemy):
    ...
    def __init__(self, level=1, name="mercenary spearman",
        characteristics=mercenary_spearman_characteristics, stats=None,
        description="The backbone of mercenary armies, a spearman. He is " +
        "well trianed and experienced.", inventory=[],
        equipment=default_equipment, tags=["human", "living", "animate",
        ...
        items.append(get_item_by_name(random.choice(["leather armor",
            "chainmail", "plate armor"]), 0)) if random.randint(0,
            10) > 3 else None
        items.append(get_item_by_name(random.choice(["iron helmet"]), 0)
            ) if random.randint(0, 10) > 3 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class MercenaryLeader(Enemy):
    ...
    def __init__(self, level=1, name="mercenary leader",
        characteristics=mercenary_leader_characteristics, stats=None,
        description="A leader of mercenaries. Hardened veteran of unfair " +
        "fights.", inventory=[], equipment=default_equipment, tags=["human",
        "living", "animate", "humanoid"], abilities=[], modifiers=[],
        exp_value=400):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name("primary weapon", 0)]
...
class Thief(Enemy):
    ...
    def __init__(self, level=1, name="thief",
        characteristics=thief_characteristics, stats=None,
        description="A professional thief.", inventory=[],
        equipment=default_equipment, tags=["human", "living", "animate",
        "humanoid", "quick"], abilities=[], modifiers=[], exp_value=400):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
        items = [get_item_by_name("dagger", 1)]
        items.append(get_item_by_name("rapier", 0)) if random.randint(0,
            10) > 5 else None
        items.append(get_item_by_name(random.choice(["steel ring",
            "golden ring", "ring"]), 0)) if random.randint(0, 10) > 8 else None
        items.append(get_item_by_name(random.choice(["bone amulet"]), 0)
            ) if random.randint(0, 10) > 8 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class MercenaryMage(Enemy):
    ...
    def __init__(self, level=1, name="mage",
        ...
        items.append(get_item_by_name(random.choice(["hermit cloak",
            "leather armor", "ritual cloak"]), 0)) if random.randint(0,
            10) > 6 else None
        items.append(get_item_by_name(random.choice(["ring", "talisman",
            "headwear"]), 0)) if random.randint(0, 10) > 6 else None
        for item in items:
            if self.add_to_inventory(item):
                self.equip(item, True)
...
class Ogre(Enemy):
    ...
    def __init__(self, level=1, name="ogre",
        characteristics=ogre_characteristics, stats=None,
        description="A slow hulking ogre. It looks hungry for you.",
        inventory=[], equipment=default_equipment, tags=["animate", "humanoid",
        "slow", "big", "humanoid", "living"], abilities=[], modifiers=[],
        exp_value=400):
        Enemy.__init__(self, name, level, characteristics, stats, description,
            inventory, equipment, tags, abilities, modifiers, exp_value)
...
def rat_pack(size=None):
    ...
    rats = [Rat(random.choice(rat_levels)) if random.randint(0,
        10) < 7 else BigRat(random.choice(rat_levels)) for x in range(amount +
        1)]
...
def wolf_pack(size=None, special_enemy=None):
    ...
    wolves = [Wolf(random.choice(wolf_levels)) for x in range(amount +
         1)] + leader
    description += desc
...
def undead_soldier_pack(size=None, special_enemy=None):
    ...
    description += desc
    ## added by ASHEN, new spawn segment ##
...
def mercenary_pack(size=None, special_enemy=None):
    ...
    if size == "small":
        ...
        amount = random.randint(4, 5)
    mercs = [Mercenary(random.choice(levels)) if random.randint(1,
        10) < 6 else MercenarySpearman(random.choice(levels)) for x in range(
        amount + 1)]
