...
class Dungeon(object):
    def __init__(self, name, description, players, enemy_types,
         rooms=[], current_room=0, difficulty=None):
        self.uid = get_uid()
        self.name = name
        ...
        if not difficulty:
            self.difficulty = sum([p.level for p in players]) \
                / len(self.players)
        else:
    ...
    def new_dungeon(players):
        ...
        dungeon = Dungeon(dungeon_name, dungeon_description,
            dungeon_players, dungeon_enemy_types)
        dungeon.generate_rooms(random.randint(settings.dungeon_room_amounts[0],
            settings.dungeon_room_amounts[1]))
        return dungeon
...
dungeons = {
    "Crypt of undead lords": {
        "description": "It's said the crypt was built" +
            " to contain a royal family turned undead.",
        "enemy_types": ["undead"]
    },
    "Underground forest": {
        "description": "A series of caves. Rich vegetation grows everywhere," +
            " as if it was a real forest. The fauna is also rich. And hostile.",
        "enemy_types": ["animal"]
    },
    "Tunnel of dimensions": {
        "description": "A series of wormholes between worlds. The horrors" +
            " that don't belong to any world infest it.",
