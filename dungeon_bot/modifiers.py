...
# Modifiers always affect only the host that carries them
    priority = 0
    duration = -1
    ...
    def get_randomized_params_for_coolity(mod_class, stats, coolity):
        ...
        for key in list(stats.keys()):
            if not key in ["tags_granted", "modifiers_granted",
                "abilities_granted"]:
                if isinstance(stats[key], list) and len(stats[key]) > 0:
                    ...
                    if isinstance(stats[key][0], int):
                        real_stats[key] = get_number_in_range(stats[key],
                            coolity)
                if isinstance(stats[key], dict):
                    for stat in stats[key]:
                        if isinstance(stats[key][stat], list) \
                            and len(stats[key][stat]) > 0:
                            if isinstance(stats[key][stat][0], str):
                                real_stats[key][stat] = \
                                    get_dice_in_range(stats[key][stat], coolity)
                            if isinstance(stats[key][stat][0], int):
                                real_stats[key][stat] = get_number_in_range(
                                    stats[key][stat], coolity)
        return real_stats
...
class KnockedDown(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="knockdown",
        description="Loose 5d6 evasion, 2d6 your defense, and half " +
        "your dexterity."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
        self.stats["characteristics_change"] = {
            "dexterity": - host.base_characteristics["dexterity"]}
        self.stats["duration"] = clamp(
            10 - host.characteristics["dexterity"], 1, 2)
...
class Vunerable(Modifier):  # simply adds defense, hinders evasion
    ...
    def __init__(self, granted_by, host, stats={}, name="vunerable",
        description="Loose 3d6 defense for a turn."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
...
class Pain(Modifier):  # simply adds defense, hinders evasion
    priority = 0
    duration = 2
    characteristics_change = {"dexterity": - 3, "strength": - 3,
        "intelligence": - 3}
    stats_change = {}
    abilities_granted = []
    tags_granted = []

    def __init__(self, granted_by, host, stats={}, name="pain",
        description="Loose 3 dexterity, strength and intelligence for a turn."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
        self.stats["characteristics_change"] = {"dexterity": - 1,
...
class Fear(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="fear",
        description="Loose 2 dexterity and intelligence."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
        self.stats["characteristics_change"] = {"dexterity": - 1,
            "intelligence": - 2}
        self.stats["duration"] = \
            clamp(10 - host.characteristics["intelligence"], 1, 3)
...
class Bleeding(Modifier):  # simply adds defense, hinders evasion
    ...
    def __init__(self, granted_by, host, stats={}, name="bleeding",
        description="loose 1d3 hp every turn for 6 rounds"):
        Modifier.__init__(self, granted_by, host, stats, name, description)
        self.stats["duration"] = \
            clamp(10 - host.characteristics["vitality"], 3, 8)
    ...
    def on_round(self):
        msg = ""
        if not self.host.dead:
            dmg = diceroll(str(int(self.host.characteristics["vitality"] / 2))
                + "d" + str(self.host.characteristics["vitality"]))
...
class Burning(Modifier):  # burning effect on the target (dot)
    priority = 0
    duration = 2
    characteristics_change = {}
    stats_change = {}
    ...
    def __init__(self, granted_by, host, stats={}, name="burning",
        description="Loose 1d9 hp every turn for 2-4 rounds and suffer an " +
        "intelligence penalty."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
        self.stats["duration"] = \
            clamp(10 - host.characteristics["vitality"] * 2, 2, 4)
...
class Shielded(Modifier):  # raises the defend of the creature
    priority = 0
    duration = 1
    characteristics_change = {}
    stats_change = {}
    ...
    def __init__(self, granted_by, host, stats={}, name="shielded",
        description="grants defense"):
        Modifier.__init__(self, granted_by, host, stats, name, description)
        if not "defense" in self.stats["stats_change"]:
            self.stats["stats_change"]["defense"] = \
                self.granted_by.stats["defense"]
        if not "evasion" in self.stats["stats_change"]:
            self.stats["stats_change"]["evasion"] = \
                self.granted_by.stats["evasion"]

    def on_applied(self):
        msg = super(Shielded, self).on_applied()
        msg += "%s gains a %s defense " % (self.host.short_desc,
            self.stats["stats_change"]["defense"])
        msg += "and %s evasion penalty for the next %d rounds.\n" % (
            self.stats["stats_change"]["evasion"], self.stats["duration"])
        msg = "!!\t" + msg
        return msg
...
class Bonus(Modifier):  # TBD

    def __init__(self, granted_by, host, stats={}, name="bonus",
        description="???"):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def get_randomized_params_for_coolity(coolity):
        ...
        stats = Modifier.get_randomized_params_for_coolity(self_class,
            stats, coolity)
...
class Nerf(Modifier):  # simply adds defense, hinders evasion
    ...
    def __init__(self, granted_by, host, stats={}, name="nerf",
        description="???"):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def get_randomized_params_for_coolity(coolity):
        ...
        stats[chars_or_stats] = {stat: stat_range}
        stats = Modifier.get_randomized_params_for_coolity(
            self_class, stats, coolity)
        #name = " ".join(stat.split("_"))
...
class Regeneration(Modifier):  # simply adds defense, hinders evasion
    ...
    def __init__(self, granted_by, host, stats={}, name="regeneration",
        ...
        if random.randint(0, 100) < chance:
            ...
            if self.host.health < self.host.stats["max_health"]:
                ...
                msg += "!!\t%s regenerates %d hp due to %s.\n" % (
                    self.host.short_desc.capitalize(), heal,
...
class Sickness(Modifier):  # simply adds defense, hinders evasion
    ...
    def __init__(self, granted_by, host, stats={}, name="sickness",
        description="Sometimes takes away health."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def get_randomized_params_for_coolity(coolity):
        ...
        stats = Modifier.get_randomized_params_for_coolity(self_class, stats,
            coolity)
...
class FireAttack(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="fire attack",
        description="Has a chance to cause fire  damage every attack by host."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def on_hit(self, attack_info):
        if attack_info.inhibitor == self.host:
            ...
            if attack_info.use_info["did_hit"] and not attack_info.target.dead \
                and not "fire resistant" in attack_info.target.tags:
                chance = diceroll(fire_chance)
                if random.randint(0, 100) < chance:
                    dmg = diceroll(fire_damage)
                    attack_info.target.damage(dmg, self.host, True)
                    attack_info.description += \
                        "!!\t%s causes %d fire damage to %s.\n" % (
                        self.granted_by.name.capitalize(),
                        dmg, attack_info.target.short_desc.capitalize())

                    chance_to_cause_burning = 1 / 2 * chance
                    if random.randint(0, 100) < chance_to_cause_burning:
                        modifier = get_modifier_by_name("burning",
                            self.granted_by, attack_info.target)
                        attack_info.use_info["modifiers_applied"].append(
                            modifier)
        return attack_info
...
class ElectricityAttack(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="electricity attack",
        description="Has a chance to cause electrical damage " +
        "every attack by host."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
...
class Energy(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="energy",
        description="Sometimes gives additional energy."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def on_round(self):
        ...
        if random.randint(0, 100) < chance:
            ...
            msg += "!!\t%s provides %s with %d energy.\n" % (
                self.granted_by.name.capitalize(),
                self.host.name.capitalize(), amount)
...
class Weakness(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="weakness",
        description="Sometimes drains energy of host."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def on_round(self):
        ...
        if random.randint(0, 100) < chance:
            ...
            msg += "!!\t%s drains %d energy from %s.\n" % (
                self.granted_by.name.capitalize(), amount,
...
class Wisdom(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="wisdom",
        description="Sometimes gives additional experience."):
        Modifier.__init__(self, granted_by, host, stats, name,
            description)
    ...
    def on_experience_gain(self, value):
        ...
        desc = ""
        if random.randint(0, 100) < chance:
            additional_gain = int(value * diceroll(
                self.stats["wisdom amount"]) / 100)
...
class Stupidity(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="stupidity",
    description="Sometimes takes away experience."):
        Modifier.__init__(self, granted_by, host, stats, name,
            description)
    ...
    def on_experience_gain(self, value):
        ...
        desc = ""
        if random.randint(0, 100) < chance:
            exp_loss = int(value * diceroll(
                self.stats["stupidity amount"]) / 100)
...
class Suffering(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="suffering",
        description="Has a chance to inflict pain to host any time."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
...
class Judgement(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="judgement",
        description="Has a chance to cause a lot of damage to any creature " +
        "in the battle at the beginning of a round. It's not clear what " +
        "judgement is behind the choice of who gets hurt, but there should " +
        "be some, right?"):
        Modifier.__init__(self, granted_by, host, stats, name, description)
    ...
    def on_round(self):
        ...
        if random.randint(0, 100) < chance:
            if self.host.event and hasattr(self.host.event,
                "turn_queue") and len(self.host.event.turn_queue) > 1:
                alive_creatues = [c for c in self.host.event.turn_queue
                    if not c.dead]
                if len(alive_creatues) > 0:
                    target = random.choice(alive_creatues)
                    dmg = diceroll(judgement_damage)
                    msg += "!!\tA lightning erupts from %s and hits %s  " % (
                     self.granted_by.name.capitalize(),
                     target.short_desc.capitalize()) + \
...
class Greed(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="greed",
        description="Sometimes destroys dropped loot."):
        Modifier.__init__(self, granted_by, host, stats, name, description)
...
#     abilities_granted = []
#     tags_granted = []
#     def __init__(self, granted_by, host, stats = {}, name="demons",
    #description="Causes demons to invade the world at unpredictable times."):
#         Modifier.__init__(self, granted_by, host,  stats, name, description)
...
class HurtUndead(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="hurt undead",
        description="More damage against undead."):
        Modifier.__init__(self, granted_by, host, stats, name, description)

    def on_hit(self, attack_info):
        if attack_info.inhibitor == self.host:
            additional_damage = int(attack_info.use_info["damage_dealt"] *
                diceroll(self.stats["additional damage to undead"]) / 100)
            if attack_info.use_info["did_hit"] and not attack_info.target.dead \
                and "undead" in attack_info.target.tags:
...
class HurtDemons(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="hurt demons",
        description="More damage against demons."):
        Modifier.__init__(self, granted_by, host, stats, name, description)

    def on_hit(self, attack_info):
        if attack_info.inhibitor == self.host:
            additional_damage = int(attack_info.use_info["damage_dealt"] *
                diceroll(self.stats["additional damage to demons"]) / 100)
            if attack_info.use_info["did_hit"] and \
                not attack_info.target.dead and \
                "demon" in attack_info.target.tags:
                attack_info.target.damage(additional_damage, self.host, True)
                attack_info.description += "!!\t%s takes additional %d " % (
                    attack_info.target.short_desc.capitalize(),
                    additional_damage) + \
...
class Vampirism(Modifier):
    ...
    def __init__(self, granted_by, host, stats={}, name="vampirism",
        description="Returns some of the damage dealt as health."):
        Modifier.__init__(self, granted_by, host, stats, name, description)

    def on_hit(self, attack_info):
        if attack_info.inhibitor == self.host and \
            self.host.health < self.host.stats["max_health"]:
            regen = int(attack_info.use_info["damage_dealt"] *
                diceroll(self.stats["vampirism amount"]) / 100)
            if attack_info.use_info["did_hit"] and not attack_info.target.dead \
                and "living" in attack_info.target.tags:
                attack_info.description += \
                    "!!\t%s regenerates %d healh because of %s.\n" % (
...
def get_random_modifiers_for_coolity(coolity):
    ...
    if got_modifier < chance_for_modifier:
        ...
        for x in range(amount_of_modifiers):
            ...
            modifier_object = modifier.get_randomized_params_for_coolity(
                coolity)
...
good_item_modifiers = ["fire attack", "electricity attack", "regeneration",
    "wisdom", "energy", "hurt undead", "hurt demons", "vampirism", "bonus"]
bad_item_modifiers = ["judgement", "suffering", "greed", "stupidity",
    "weakness", "nerf"]